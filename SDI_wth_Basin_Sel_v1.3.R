...
server <- function(input, output, session) {
  ...
  # 2) Pull basin list + coords from basin_meta_db
  ...
  # 3) Render basin map & track clicks
  ...
  # 4) Fetch station IDs for that basin from basin_st_tab
  ...
  # 6) Query SDI from stn_sf_tab based on inputs
  combined_data <- reactive({
    ...
    ed      <- paste0(input$year_range[2], "-12-31")
