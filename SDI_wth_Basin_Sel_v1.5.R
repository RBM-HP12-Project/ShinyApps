      ...
      tabItem(tabName = "upload_tab",
              fluidRow(
                box("Upload Daily River Flow CSV", status = "primary", solidHeader = TRUE, width = 6,
                    fileInput("file", "Choose CSV:", accept = c(".csv")),
                    selectInput("upload_aggregation", "Aggregation:", choices = c("3-month", "6-month")),
                    dateInput("upload_ref_date", "Reference Date:", value = Sys.Date()),
...
server <- function(input, output, session) {
  ...
  # 2) Load basin coords + shapes from DB + shapefile
  ...
  observeEvent(input$basin_map_shape_click, {
    selected_basin(input$basin_map_shape_click$id)
